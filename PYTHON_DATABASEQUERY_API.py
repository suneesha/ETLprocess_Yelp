...
def getlabelled_entries(category,k):
    ...
    l=[]
    for item in cursor:
        l.append('\t'.join([str(item[0]),str(item[1]),str(haversine(gps[1],gps[0],float(item[6]),float(item[5])))]))
    i=0
    if len(l)==0:
...
if __name__=="__main__":    
    ...
    while process!='N':
        ...
        if input[0]=='searchtag':
            print getsearchtag_entries(input[1],int(input[2]))
        if input[0]=='label':
            print getlabelled_entries(input[1],int(input[2]))
